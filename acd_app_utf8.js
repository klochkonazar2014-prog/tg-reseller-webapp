            ...
            const processed = items.map(item => {
                ...
                if (item.metadata) {
                    try {
                        ...
                        item._modelName = m.model || null;
                        item._collection = { name: m.collection || "Gifts" };
                    } catch (e) {
...
function createItemCard(item) {
    ...
    const renderMediaHTML = (it, isModal = false) => {
        ...
        if (isNum || isUser) {
            ...
            // If still no image, try Fragment URLs again (this covers cases where _realImage was initially null/placeholder)
...
function generateFragmentUrls(n, attempt = 0) {
    ...
    let slug = SLUG_MAPPING[lookupKey] || lookupKey;
...
async function loadHistoryContent() {
    ...
    try {
        ...
        orders.forEach(o => {
            ...
            if (o.status === 'active') { statusColor = '#34C759'; statusText = '╨Р╨║╤В╨╕╨▓╨╡╨╜'; }
            if (o.status === 'paid') { statusColor = '#007AFF'; statusText = '╨Ю╨▒╤А╨░╨▒╨╛╤В╨║╨░...'; }
        ...
        <div style="display:flex; justify-content:space-between; font-size:12px; color:#8b9bb4;">
            <span>╨б╤А╨╛╨║: ${o.days} ╨┤╨╜.</span>
            <span>${o.total_price} TON</span>
        </div>
        ${o.status === 'rented' ? `
            <button onclick="openTcModal(${o.id})" class="btn-yellow" style="height:36px; font-size:12px; margin-top:5px;">╨Ю╤В╨┐╤А╨░╨▓╨╕╤В╤М tc:// ╤Б╤Б╤Л╨╗╨║╤Г</button>
        ` : ''}
    `;
